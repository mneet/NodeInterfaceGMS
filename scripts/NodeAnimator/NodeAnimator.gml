function __node_animator() constructor
{
	...
	call_at_end_delay = array_create(NODE_MOTION_STATE.LENGTH, 0);
	event_state = [];
	...
	state_motion_enter = function(_node, _animator)
	{
		if (!motion_state_set)
		{
			__animator_execute_events(NODE_MOTION_STATE.ENTER);
	...
	state_motion_leave = function(_node, _animator)
	{
		...
		state_timer += delta_time_seconds();
		
		if (state_timer >= call_at_end_delay[NODE_MOTION_STATE.LEAVE])
		{
			call_function_when_complete();
			if (!restart_anim)
			{
	...
	///@function		set_default_anim_alpha(_state, _curve, _value_to, _timer_max, _delay)
	static set_default_anim_alpha = function(_state, _curve, _value_to, _timer_max = 0.3, _delay = 0)
	{
		var _controller =  new __node_animator_ac_control(_curve, _timer_max, _delay);
		...
		// Define animação padrão de escala
		set_state_motion(_state, global.node_animator_db.simple_alpha_animation);
	...
	///@function		set_state_call_at_end_delay(_state, _delay)
	///@description		Set delay to call at end function (Only works with default states)
	static set_state_call_at_end_delay = function(_state, _delay)
	{
		call_at_end_delay[_state] = _delay;
	}
	...
	static animator_bind_event = function(_state, _event_funct, _delay_at_end = 0)
	{
		call_at_end_delay[_state] = _delay_at_end;
...
{	
	...
	{		
		...
		{
			if (_event_ctrl.timer_delay > 0)
			{
				_event_ctrl.timer_delay -= delta_time_seconds();
				return;
			}
			...
			// Aplicando escala no node
			_node.transform.local.scale.x = _scale_target;
			_node.transform.local.scale.y = _scale_target;
			
			// Atualizando transform dos nodes aninhados
			_node.transform.__transform_recalculate();
	...
	simple_alpha_animation = function(_node, _event_ctrl)
	{		
		// Checa se a animação já completou
		if (!_event_ctrl.animation_completed)
		{
			...
			{
				// Valor inicial no momento que a animação começou
				_event_ctrl.value_start = variable_clone(_node.transform.alpha);
			...
			// Utilizando função de Easing do script PennersEasingAlgorithms
			var _alpha_target = _easing_func(_event_ctrl.timer, _event_ctrl.value_start, _event_ctrl.value_diff, _event_ctrl.timer_max);	
			
			// Aplicando escala no node
			_node.transform.alpha = _alpha_target;

			// Aplicando escala no node
			_node.transform.local.alpha = _alpha_target;
